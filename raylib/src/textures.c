...
// Generate image: smooth white noise
Image GenImageSmoothWhiteNoise(int width, int height, float factor)
{
    Color* pixels = (Color*)RL_MALLOC(width * height * sizeof(Color));

    for (int i = 0; i < width * height; i++)
    {
        pixels[i].r = GetRandomValue(0, 254);
...
// Crop image depending on alpha value
// NOTE: Threshold is defined as a percentatge: 0.0f -> 1.0f
void ImageGrayscaleCrop(Image* image, float threshold)
{
    // Security check to avoid program crash
    if ((image->data == NULL) || (image->width == 0) || (image->height == 0)) return;

    Rectangle crop = GetImageGrayscaleBorder(*image, threshold);
...
// Get image alpha border rectangle
// NOTE: Threshold is defined as a percentatge: 0.0f -> 1.0f
Rectangle GetImageGrayscaleBorder(Image image, float threshold)
{
    Rectangle crop = { 0 };
    ...
    if (pixels != NULL)
    {
        ...
        for (int y = 0; y < image.height; y++)
        {
            for (int x = 0; x < image.width; x++)
            {
                if (pixels[y * image.width + x].r > (unsigned char)(threshold * 255.0f))
                {
                    if (x < xMin) xMin = x;
                    if (x > xMax) xMax = x;
                    if (y < yMin) yMin = y;
