...
RLAPI void rlRotateAngle(float x, float y, float z);  // Multiply the current matrix by a rotation matrix
...
// Multiply the current matrix by a rotation matrix
void rlRotateAngle(float x, float y, float z)
{
    Matrix matRotation = MatrixIdentity();

    //Vector3 axis = (Vector3){ x, y, z };
    //matRotation = MatrixRotate(Vector3Normalize(axis), angleDeg * DEG2RAD);
    Vector3 angle = (Vector3){ z * DEG2RAD,y * DEG2RAD,x * DEG2RAD };
    matRotation = MatrixRotateZYX(angle);
    // NOTE: We transpose matrix with multiplication order
    *RLGL.State.currentMatrix = MatrixMultiply(matRotation, *RLGL.State.currentMatrix);
}
