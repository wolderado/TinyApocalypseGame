...
// Draw cube
// NOTE: Cube position is the center position
void DrawCubeRotated(Texture texture,Vector3 position, float width, float height, float length,float angle,Vector3 axis, Color color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
// Draw cube
// NOTE: Cube position is the center position
void DrawCubeRotatedAngle(Texture texture, Vector3 position, float width, float height, float length, float angleX, float angleY, float angleZ, Color color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
    ...
    rlTranslatef(position.x, position.y, position.z);
    rlRotateAngle(angleX, angleY, angleZ);
...
// Draw cube
// NOTE: Cube position is the center position
void DrawCubeRotatedOneSidedTexture(Texture texture, Vector3 position, float width, float height, float length, float angle, Vector3 axis, Color color)
{
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
// Draw cube
// NOTE: Cube position is the center position
void DrawPlaneTexture(Texture2D texture, Vector3 position, float width, float length, Color color)
{
    float x = position.x;
    float y = position.y;
    float z = position.z;
    ...
    rlBegin(RL_QUADS);
        ...
        rlTexCoord2f(0.0f, 0.0f); rlVertex3f(x - width / 2, y, z + length / 2);  // Bottom Left Of The Texture and Quad
        rlTexCoord2f(1.0f, 0.0f); rlVertex3f(x + width / 2, y, z + length / 2);  // Bottom Right Of The Texture and Quad
        rlTexCoord2f(1.0f, 1.0f); rlVertex3f(x + width / 2, y, z - length / 2);  // Top Right Of The Texture and Quad
   rlEnd();
        //rlPopMatrix();
...
// Draw a flipped plane
void DrawFlippedPlane(Vector3 centerPos, Vector2 size, Color color)
{
    rlCheckRenderBatchLimit(4);
